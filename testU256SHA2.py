...
class TestSHA2 (unittest.TestCase):
    ...
    def testCopyAndPut(self):
        """we are testing sha1Key = u256.copyAndPut2(path, uPath, key) """

        # create a random file                          maxLen   minLen
        (dLen, dPath) = self.rng.nextDataFile(DATA_PATH, 16 * 1024, 1)
        dKey = u256.fileSHA2Hex(dPath)

        # invoke function
        (uLen, uKey) = u256.copyAndPut2(dPath, U_PATH, dKey)
        ...
        dKeyBin = u256.fileSHA2Bin(dPath)
    ...
    def testPut(self):
        ...
        (uLen, uKey) = u256.put2(dPath, U_PATH, dKey)
        uPath = u256.getPathForKey(U_PATH, uKey)
        ...
        (dupeLen, dupeKey) = u256.put2(dupePath, U_PATH, dKey)
    ...
    def testPutData(self):
        ...
        (uLen, uKey) = u256.putData2(data, U_PATH, dKey)
        self.assertEqual(dKey, uKey)
