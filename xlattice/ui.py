...
def passwdStrength(p):
    ...
    if length == 0:
        ...
        charSetSize = 0
        if re.search(r'[a-z]', p):  charSetSize += 26
        if re.search(r'[A-Z]', p):  charSetSize += 26
        if re.search(r'\d',    p):  charSetSize += 10
        if re.search(r'[^\w]', p):  charSetSize += 16  # say ;-)

        if length >= 12:
            if charSetSize >= 52:   est = 'very strong'
            else:                   est = 'strong'
        elif length >= 10:
            if charSetSize >= 52:   est = 'strong'
            else:                   est = 'medium'
        elif length >= 8:
            if charSetSize >= 52:   est = 'medium'
            else:                   est = 'fair'
        else:
            if charSetSize >= 78:   est = 'fair'
            elif charSetSize >= 52: est = 'poor'
            else:                   est = 'weak'
